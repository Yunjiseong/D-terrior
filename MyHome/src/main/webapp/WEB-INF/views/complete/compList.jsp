    ...
    <div class="container">      
      <div class="control-list">  
		...
		<div id="sorting">
		    ...
		    <select class="input-box" name="place">
                <option value selected>시공영역</option>
                <option>전체</option>
                <option>주방</option>
                <option>화장실</option>
                <option>조명</option>
	        ...
	        <select class="input-box" name="money2">          
	            <option value="0" selected>가격</option>
	            <option value="1">500만원 미만</option>
	            <option value="2">500~1000만원</option>
	            <option value="3">1000~1500만원</option>
	            <option value="4">1500~2000만원</option>
	            <option value="5">2000~3000만원</option>
	            <option value="6">3000~4000만원</option>
	            <option value="7">4000~5000만원</option>
	            <option value="8">5000만원 이상</option>
	        </select>  
	        
	       <c:if test="${sessionScope.user != null}"> 
	       	<button type="button" id="writeBtn" class="btn btn-info float-right" onclick="location.href='<c:url value="/comp/compWrite" />'">글쓰기</button> 
	       </c:if>
		</div>     
...
<script>
	$(function() { //start jquery
		...
		function getList(page, reset) {
			...
			const place = $('select[name=place] option:selected').val();
			const homeSize = $('select[name=homeSize] option:selected').val();
			const money = $('select[name=money2] option:selected').val();
